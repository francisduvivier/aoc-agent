# Things to improve
- solution for git commits, currently, the problem is that the agent cannot make commits because docker cannot access the .git folder. We should instead make a separate git dir out of the solutions dir and ignore it in the parent git repo. Or for now, we can just get rid of the rw folder, the only thing we are currently shielding is the .gitignore file, which does not matter so much.
