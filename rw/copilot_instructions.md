...
- the agent code should create a directory for the current day and download the input file using a curl request with a session cookie that I have already put in the environment as AOC_SESSION_COOKIE=[value after session=]. Place this and other env vars in a .env file at the project root for docker compose to load (docker compose will automatically read .env when env_file is set).
- the agent should run in a docker environment, only having access to a mounted directory; for that purpose, a docker compose should be made (use the "docker compose" command with a space, not "docker-compose").
...
- the agent should OpenRouter, the api key is provided in the env as AOC_OPENROUTER_API_KEY, and you should use the grok 4.1 fast free model as the model for the agent.
