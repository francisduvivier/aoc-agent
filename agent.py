...
def main():
    ...
    parser.add_argument("--run-only", action="store_true",
                        help="Only run existing scaffold/solution and attempt submission; do not fetch")
                        ...
                        try:
                            ...
                            if proc.returncode == 0 and proc.stdout.strip():
                                ...
                                if sample_match and final_match:
                                    ...
                                    if test_res != real_res and test_res != "0" and real_res != "0":
                                        output = real_res
                                        logging.info("Output from generated solution_part1.py: Test=%s, Real=%s",
                                                     test_res, real_res)
                                    else:
        ...
        if status['part1_solved']:
            ...
            if not status['part2_solved']:
                ...
                for attempt in range(1, max_attempts + 1):
                    ...
                    if not output and api_key:
                        ...
                        if code:
                            with open(sol2, "w") as f:
                                f.write(code)
                            git_commit([sol2],
                                       f"Agent generated solution_part2 for {year} day {day} (attempt {attempt})")
                            try:
                                ...
                                if proc.returncode == 0 and proc.stdout.strip():
                                    ...
                                    if sample_match and final_match:
                                        ...
                                        if test_res != real_res and test_res != "0" and real_res != "0":
                                            output = real_res
                                            logging.info("Output from generated solution_part2.py: Test=%s, Real=%s",
                                                         test_res, real_res)
                                        else:
                                            feedback = f"Generated solution output invalid: Test='{test_res}', Real='{real_res}'. Must be distinct and non-zero."
                                            previous_code = code
                    ...
                    if attempt == max_attempts:
                        logging.error("Part 2 failed after %d attempts.", max_attempts)
                ...
                if not known_answer:
                    ...
                    for attempt in range(1, max_attempts + 1):
                        ...
                        if code:
                            ...
                            try:
                                proc = subprocess.run(["python3", "solution_part2_verify.py"], cwd=workdir,
                                                      capture_output=True, text=True, timeout=60)
                                if proc.returncode == 0 and proc.stdout.strip():
                                    ...
                                    if sample_match and final_match:
                                        ...
                                        if output == known_answer:
                                            ...
                                            git_commit([os.path.join(workdir, "solution_part2.py")],
                                                       f"Agent verified solution_part2 for {year} day {day}")
                                            break
                                        else:
                                            logging.warning(
                                                "VERIFICATION FAILED: Generated answer '%s' != Known answer '%s'",
                                                output, known_answer)
                                            feedback = f"Generated answer '{output}' is incorrect. Expected '{known_answer}'."
                                            previous_code = code
                                    else:
                                        logging.warning(
                                            "Verification script produced insufficient output (missing format markers). Output:\n%s",
                                            proc.stdout)
